...
//                                        System.out.println(data);
    ...
    public void wirteExcel(String titleName[], CellType cellType[], int titleSize[], List<String[]> dataList) {
		// 添加Worksheet（不添加sheet时生成的xls文件打开时会报错)
		Sheet sheet = workbook.createSheet(this.sheetName);
		// 新建文件
		OutputStream out = null;
		try {
			...
			if (dataList != null && dataList.size() > 0) {
				...
				if (titleName.length > 0) {
					for (int rowIndex = 1; rowIndex <= dataList.size(); rowIndex++) {
						String[] datas = dataList.get(rowIndex - 1); // 获得该对象
						Row dataRow = workbook.getSheet(sheetName).createRow(rowIndex);
						for (int columnIndex = 0; columnIndex < titleName.length; columnIndex++) {
							String title = titleName[columnIndex].toString().trim();
							if (!"".equals(title)) { // 字段不为空
								
								String data = datas[columnIndex];
								Cell cell = dataRow.createCell(columnIndex);
