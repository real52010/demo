...
public class DataTransApp2 {

	// private final static Logger log =
	// LoggerFactory.getLogger(DataTransApp.class);

	private static String inputDir = "E:/00.个人目录/贷款用户/batch3/";// 指定程序执行结果保存的文件路径
	private static String outPutDir = "E:/00.个人目录/贷款用户/batch3-result/";// 指定程序执行结果保存的文件路径
	private static String logFileDir = "E:/00.个人目录/贷款用户/batch3-result/";// 指定程序执行结果保存的文件路径
	...
	public static List<String> loadExcelData(File file) {
		...
		for (ExcelSheet excelSheet : excelList) {
			...
			sheetmsg.append(" 其中有效数据：" + sheetDataList.size() + "条");
	...
	public static void main(String[] args) throws IOException {
		...
		for (File file : files) {
			...
			Map<String, int[]> fileMap = new HashMap<String, int[]>();
			...
			for (Iterator iterator = fileMap.keySet().iterator(); iterator.hasNext();) {
				...
				writemsg(String.format("已写至文件%s,第%s行-第%s行.", fileName, (fileCountSign[0] + 1) + "",
						fileCountSign[1] + ""));
